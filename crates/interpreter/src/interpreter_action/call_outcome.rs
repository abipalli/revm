...
impl CallOutcome {
    ...
    /// Returns a reference to the output_guint data.
    ///
    /// Provides access to the garbled output data generated by the exected instruction.
    ///
    /// # Returns
    ///
    /// A reference to the garbled output data as [`GarbledUint256`].
    pub fn output_guint(&self) -> Option<&GarbledUint256> {
        if let Some(output) = &self.result.output_guint {
            Some(output)
        } else {
            None
        }
